...
async function callOpenAI({ today, candidates, topicWeights, comments, previousError = '' }) {
  ...
  const body = {
    ...
    text: {
      format: {
        type: 'json_schema',
        name: 'daily_briefing_selection_v10_11',
        strict: true,
...
function completeAnalysisFields(selection, candidates) {
  ...
  // IMPORTANT: Do not use flattenSelection() here. It returns cloned objects,
  // so edits would not be written back to selection.work_items/society_items.
  // Iterate over the original arrays and attach section temporarily.
  const groups = [
    ['work', Array.isArray(selection?.work_items) ? selection.work_items : []],
    ['society', Array.isArray(selection?.society_items) ? selection.society_items : []]
  ];
